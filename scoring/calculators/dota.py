...
DOTA_SPLIT = 0.7  # 70% of the score is rank, 30% is from individual character scores
...
class DotaScoreCalculator(ScoreCalculator):
    ...
    def calculate_score(self, user: User) -> int:
        ...
        rank_score = (
            (2 ** (rank_tier / 10))
            * (COMP_MAX_SCORE / (80**DOTA_COMP_GROWTH_EXP))
            * DOTA_SPLIT
        )
        ...
        response = requests.get(
            "https://api.opendota.com/api/players/" + str(dotaID) + "/rankings"
        )
        char_score = 0
        for hero in response.json():
            prc = hero["percent_rank"]
            if prc > 0.6:
                score = ((prc - 0.6) * 100) * DOTA_CHAR_GROWTH_Factor
            else:
                score = 0
            char_score += score
            # raw_score += (10000 / val**GROWTH_EXP) + BONUS_PTS
        max_char_score = COMP_MAX_SCORE * (1 - DOTA_SPLIT)
        if char_score > max_char_score:
            char_score = max_char_score
